...
* Поисковые запросы только в фокусе: *«вайб кодинг финансы Cursor»*, *«MCP Cursor 1С API»*, *«HTTP сервис 1С интеграция»*, *«Google Sheets API финотдел»*, *«приложение для финансиста»*.
* **Не ищи и не предлагай** Make / n8n / Zapier.
...
### Шаг 4 — Тест на каннибализацию (job-to-be-done, не перефраз)
...
```
Если exit code 1 / `OVERLAP DETECTED` — тему НЕ добавляй. **Перефраз того же смысла запрещён** (та же работа другими словами = дубль).  
Запрещённые кластеры уже закрытых тем: выписка/staging, сверка банк↔учёт, план-факт, 1С→Excel/OData, ИИ/Cursor→формулы Excel, дайджест собственнику, дебиторка+напоминания, Make vs n8n.
**Вне фокуса Scout:** любые новые темы про Make / n8n / Zapier — пропускай. Фокус: вайб-кодинг, интеграции, MCP, API, веб-сервисы, приложения.
...
* Темы про Make / n8n / Zapier — вне фокуса. Фокус: вайб-кодинг, интеграции, MCP, API, веб-сервисы, приложения.
