#!/usr/bin/env python3
"""Загрузка файла dzen_payload.js на временный HTTPS-хостинг tmpfiles.org."""
...
def main() -> int:
    ...
    print("Uploading dzen_payload.js to tmpfiles.org...")
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as res:
            ...
            if resp.get("status") == "success":
                # Ссылка для скачивания (нужно заменить tmpfiles.org на tmpfiles.org/dl)
                file_url = resp["data"]["url"]
                dl_url = file_url.replace("tmpfiles.org/", "tmpfiles.org/dl/")
                print(f"SUCCESS: Uploaded to {dl_url}")
                return 0
            else:
                print("Error: status not success", resp)
                return 1
